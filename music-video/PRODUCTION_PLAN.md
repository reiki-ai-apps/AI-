...
**契約の判断材料が変わった。** 当初は「動画生成(Kling)のために Starter $19」という話だったが、
実際に詰まったのは**静止画の絵柄**のほうだった。

## 何が効くか（実測）

| | モデル | 1枚 | 無料枠 | 役割 |
|---|---|---|---|---|
| 構図出し | `soul_cinematic` | 約 0.12 | ○ | 机・光・配置を決める。安いので何枚でも振れる |
| **絵柄** | **`flux_kontext`** | **約 1.5** | **○** | **写真をアニメ背景美術に描き直す。ここが本命** |
| 生成系 | `seedream_v4_5` / `flux_2` / `nano_banana` / `soul_location` | — | **×** | `Requires basic plan or higher` |
| 動画 | `kling3_0` ほか | 約 14 | 未確認 | 8秒クリップ |

プロンプトでアニメ背景美術を**直接生成するのは無理だった**。実写寄りのモデルしか無料枠に無く、
`soul_cinematic` に絵柄を指示しても写真か抽象画にしかならない。
当たったのは **flux_kontext で描き直す**2段構え。詳細は `not-your-medicine/ART_DIRECTION.md`。

## 無料枠の限界

10 クレジットだと **flux_kontext は5〜6枚で尽きる**。1曲4カットぶんは通るが、
描き直しのやり直しはできない。実際 NOT YOUR MEDICINE で 8.44 → 0.82 まで使い切った。

つまり契約する理由は「動画を作るため」ではなく、**絵柄を詰めるため**。
1曲あたり flux_kontext を10〜15枚は振りたい（＝15〜23クレジット）。

## 役割分担（変わらず）

| 誰が | 何を |
|---|---|
| Higgsfield | 背景美術（構図→描き直しの2段） |
...
python3 common/compose.py --spec not-your-medicine/shots.json \
  --song NOT_YOUR_MEDICINE.mp3 --grid-json grid.json --out nym.mp4
```

`--stills` は `shots.json` と同じ**絶対秒**で指定する（相対秒ではない）。

---

以下は当初の検討（動画中心の前提。上の実測で置き換わっている）。
