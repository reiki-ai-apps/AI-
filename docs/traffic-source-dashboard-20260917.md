...
## 本番DBの有効化（2026-09-18追記：適用済み）

本番 Supabase の ncosmmesecpqhzfikpmn には、2026-09-17に `20260917125507 / access_insights_preserve_history_operator_only` が適用済み。運営者の判定と UUID 再送処理を強化した版であり、このリポジトリーの旧 `supabase/access-insights-2026-09-17.sql` を再適用して上書きしないこと。

接続が確認できるまでは実数が見えるとは報告しない。SQL未適用の画面は理由を表示し、流入元の値は「—」。0回を装わない。過去に流入元を記録していないアクセスは復元できず「過去の未記録」として扱う。

画面と計測リンクの最新の説明は [2026-09-18の改善記録](traffic-source-clarity-20260918.md) を参照。
